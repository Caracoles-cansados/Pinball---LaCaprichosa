...
bool ModuleFadeToBlack::Start()
{
	...
	fadeInOut_texture = App->textures->Load("textures/fade.png");
...
update_status ModuleFadeToBlack::PostUpdate()
{
	...
	if (currentStep == Fade_Step::TO_BLACK) {
		...
		App->renderer->Blit(fadeInOut_texture, App->renderer->camera.x / SCREEN_SIZE - 320, 0, &rect, SDL_FLIP_NONE);
		App->renderer->Blit(fadeInOut_texture, App->renderer->camera.x / SCREEN_SIZE, 0, &rect, SDL_FLIP_NONE);
		App->renderer->Blit(fadeInOut_texture, App->renderer->camera.x / SCREEN_SIZE + 320, 0, &rect, SDL_FLIP_NONE);
	}
	else {
